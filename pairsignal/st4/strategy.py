...
def deviation_gate(side: Signal, cur: float, sma: float, cfg: StrategyConfig) -> bool:
    ...
    AbsOfMean (по умолчанию, корректно при любом знаке SMA):
        ...
        BUY:  SMA − cur >= DeviationPct·|SMA|
    LiteralPct (буквально из исходного ТЗ, ломается при SMA<0):
        SELL: cur >= SMA·(1 + DeviationPct);  BUY: cur <= SMA·(1 − DeviationPct)
    """
    p = cfg.deviation_pct
...
def entry_signal(prev: BandReading, cur: BandReading, cfg: StrategyConfig) -> Signal:
    ...
    SELL спреда (шорт SBRF + лонг SBPR): prev < Upper и cur >= Upper (пробой вверх).
    BUY  спреда (лонг SBRF + шорт SBPR): prev > Lower и cur <= Lower (пробой вниз).
    """
    if not (cur.is_ready and prev.is_ready):
        return Signal.NONE
    # пробой верхней полосы → SELL
    if prev.spread < prev.upper and cur.spread >= cur.upper:
        if deviation_gate(Signal.SELL, cur.spread, cur.sma, cfg):
            return Signal.SELL
    # пробой нижней полосы → BUY
    if prev.spread > prev.lower and cur.spread <= cur.lower:
        if deviation_gate(Signal.BUY, cur.spread, cur.sma, cfg):
            return Signal.BUY
    return Signal.NONE
